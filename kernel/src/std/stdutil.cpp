...
namespace std
{
    ...
    template <typename T>
    shared_ptr<T>::shared_ptr(T* p) : ptr(p), ref_count(new size_t(1)) {}
    ...
    template <typename T>
    shared_ptr<T>::shared_ptr(const shared_ptr& other) : ptr(other.ptr), ref_count(other.ref_count) {
        ++(*ref_count);
    }
    ...
    template <typename T>
    shared_ptr<T>::shared_ptr(shared_ptr&& other) noexcept : ptr(other.ptr), ref_count(other.ref_count) {
        other.ptr = nullptr;
