...
extern void* liballoc_alloc(size_t c) {
    size_t count = c;
    size_t idx = 0, pages_found = 0;
    while (idx < kheap::kernel_heap_max_size_pages) {
        if (BITMAP_READ_BIT(kheap::kernel_heap_bitmap, idx) == 0) {
            pages_found++;
...
extern int liballoc_free(void* address, size_t count) {
    for (size_t i = 0; i < count; i++) {
        kheap::return_page_at(((uintptr_t)address + (i * PAGE_SIZE)));
    }
    return 1;
}
